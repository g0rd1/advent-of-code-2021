...
fun main() {
    ...
    val (winBoard, markedNumbersCount) = getLastWinBoardAndMarkedNumbersCount(winNumbers, boards)
    val sumOfAllUnmarkedNumbers = winBoard
        .flatten()
...
private fun getLastWinBoardAndMarkedNumbersCount(
    ...
    for (markedNumbersCountToCheck in 1..winNumbers.size) {
        val winBoard = boards.firstOrNull { isWinBoard(it, winNumbers.take(markedNumbersCountToCheck)) }
        if (winBoard != null) {
            return winBoard to markedNumbersCountToCheck
        }
    }
