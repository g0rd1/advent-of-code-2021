...
fun main() {
    ...
    println(getFirstStepDuringWhichAllOctopusesFlash(energy))
}

private fun getFirstStepDuringWhichAllOctopusesFlash(energy: List<List<Energy.StoreEnergy>>): Long {
    var newEnergy = energy.toList()
    var counter = 0L
    while (newEnergy.flatten().any { it.energy != 0 }) {
        newEnergy = getEnergyAndFlashesAfterSteps(newEnergy, 1).first
        counter++
    }
    return counter
}
